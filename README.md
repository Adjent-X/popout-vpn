...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-0.2.0--beta-06b6d4" />
  ...
  <img alt="Status" src="https://img.shields.io/badge/status-beta-67e8f9" />
</p>

Popout VPN is a self-hosted **OpenVPN + WireGuard control plane**: client configs, live connections, analytics, attack capture, traffic shaping, and a dark admin UI. The installer has the **same Linux coverage as [Angristan OpenVPN](https://github.com/angristan/openvpn-install)** — it runs that installer, then [Angristan WireGuard](https://github.com/angristan/wireguard-install), dual OpenVPN (UDP + TCP), a private bind IP with persisted DNAT, and the rest of the stack.

This is **v0.2.0-beta**. The [roadmap](ROADMAP.md) still has packages, Docker, and multi-node. Star the repo if you want that work to stay loud.
...
<p align="center">
  ...
  <sub><strong>VPN configs.</strong> Issue or import seats, then download <code>.ovpn</code> (OpenVPN) or <code>.conf</code> (WireGuard). Watch live tunnel sessions (VPN IP + last WAN), expiry, and per-client WAN logging. Duplicate-CN mode is optional when one cert is shared by many seats.</sub>
</p>
...
- **OpenVPN that already works** — Angristan’s installer on Debian, Ubuntu, Fedora, Rocky / Alma / CentOS Stream, Amazon Linux, Oracle Linux, and Arch. Existing Nyr layouts are detected. Beta runs **UDP and TCP concurrently**.
- **WireGuard** — Angristan’s installer; peers are issued with the same seat as OpenVPN. Download `.conf` or `.ovpn`.
- **Private bind + DNAT** — servers listen on `10.255.255.1`; iptables `-j DNAT --to-destination` is persisted and follows enable/disable toggles.
- **Admin portal** — Next.js + FastAPI. Issue and revoke configs, invite staff, scoped API keys, live sessions.
...
- **Multiport / knock-style remotes** — ipset `ephemeral-ports` (45000–45099) plus iptables DNAT to the real OpenVPN ports. Clients can advertise `remote-random`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Adjent-X/popout-vpn/beta/install.sh -o popout-vpn-install.sh
sudo bash popout-vpn-install.sh
```

Stable (OpenVPN-only) remains on `main`. This **beta** branch adds WireGuard, dual OpenVPN, and DNAT.

Or clone and run from the tree:

```bash
git clone --branch beta https://github.com/Adjent-X/popout-vpn.git
cd popout-vpn
...
2. **Install OpenVPN via Angristan** if it is not already there, then split it into concurrent UDP + TCP instances bound to a private IP.
3. **Install WireGuard via Angristan** if it is not already there.
4. Ask: **Put the admin portal behind Cloudflare? [y/N]**  
   If yes: apex domain + API token (or Global API Key + email). See [docs/CLOUDFLARE.md](docs/CLOUDFLARE.md).
5. Generate JWT, Mongo password, and the first admin login.
6. Persist iptables `-j DNAT --to-destination` toward the bind IP, plus ipset `ephemeral-ports`.
7. If Cloudflare was enabled, auto-tune the zone (DNS, SSL, cache, WAF).
8. Print the portal URL and default login.
